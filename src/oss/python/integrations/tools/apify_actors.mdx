...
Apify uses pay-per-use pricing with a free tier. Some Actors are free (you only pay for platform compute), while others charge per result or event.
...
When invoked by an LLM agent, `run_input` is passed as a JSON-encoded string (the tool's `args_schema` types it as `str`). When invoked directly from Python, a dict works as well; the tool accepts either form.
